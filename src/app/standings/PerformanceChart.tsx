// src/app/standings/PerformanceChart.tsx
"use client";
...
/** dot renderer factory: small dot for points, team logo on the last point */
const makeDot =
  (team: Team, color: string, lastIndex: number) =>
  (props: any) => {
...
export default function PerformanceChart({
  ...
  const { rows, yMax } = useMemo(() => {
    ...
    const lastWeek = finished.length
      ...
      : 1; // kalau belum ada hasil, paksa W1 biar chart tidak kosong

    // 2) Hitung kemenangan per minggu
    ...
    // 3) Susun data kumulatif per minggu untuk setiap tim
        ...
        <div className="hidden sm:flex flex-wrap gap-x-4 gap-y-1 text-xs">
          {TEAMS.map((t) => (
            <span key={t} className="inline-flex items-center gap-1">
              <span className="inline-block w-2 h-2 rounded-full" style={{ background: palette[t] }} />
              {t}
            </span>
          ))}
        </div>
